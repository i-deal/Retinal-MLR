...
class AIR(nn.Module):
    def __init__(
        ...
        self.x_size = 64 #x_size
        self.window_size = 28 #window_size
        ...
        # Create nn modules.
        rnn_input_size = (self.x_size**2)*3 if embed_net is None else embed_net[-1]
        rnn_input_size += self.z_where_size + self.z_what_size + self.z_pres_size
        nl = getattr(nn, non_linearity)

        self.rnn = nn.LSTMCell(rnn_input_size, rnn_hidden_size)
        self.encode = Encoder((window_size**2)*3, encoder_net, z_what_size, nl)
        self.decode = Decoder(
            (window_size**2)*3,
            decoder_net,
            z_what_size,
    ...
    def prior(self, n, **kwargs):
        state = ModelState(
            x=torch.zeros(n,3, self.x_size, self.x_size, **self.options),
        ...
        for t in range(self.num_steps):
            lst = self.prior_step(t, n, state, **kwargs)
            state, y = lst[0], lst[1]
            z_where.append(state.z_where)
            z_pres.append(state.z_pres)
            y_lst += [y]

        return (z_where, z_pres), state.x, y_lst
    ...
    def prior_step(self, t, n, prev, z_pres_prior_p=default_z_pres_prior_p):
        ...
        x = prev.x + (y * z_pres.view(-1, 1, 1, 1))

        return [ModelState(x=x, z_pres=z_pres, z_where=z_where), y]
    ...
    def model(self, data, batch_size, **kwargs):
        ...
        with pyro.plate("data", data.size(0), device=data.device) as ix:
            batch = data[ix]
            n = batch.size(0)
            (z_where, z_pres), x, objs = self.prior(n, **kwargs)
            pyro.sample(
                "obs",
                dist.Normal(
                    x.view(n, -1),
                    (
                        self.likelihood_sd
                        * torch.ones(n, 3*(self.x_size**2), **self.options)
                    ),
                ).to_event(1),
                obs=batch.view(n, -1),
            )
...
def window_to_image(z_where, window_size, image_size, windows):
    n = windows.size(0)
    assert windows.size(1) == (window_size**2)*3, "Size mismatch."
    theta = expand_z_where(z_where)
    grid = F.affine_grid(theta, torch.Size((n, 3, image_size, image_size)))
    out = F.grid_sample(windows.view(n, 3, window_size, window_size), grid)
    return out.view(n, 3, image_size, image_size)


def image_to_window(z_where, window_size, image_size, images):
    n = images.size(0)
    assert images.size(2) == images.size(3) == image_size, "Size mismatch."
    theta_inv = expand_z_where(z_where_inv(z_where))
    grid = F.affine_grid(theta_inv, torch.Size((n, 3, window_size, window_size)))
    out = F.grid_sample(images.view(n, 3, image_size, image_size), grid)
    return out.view(n, -1)
